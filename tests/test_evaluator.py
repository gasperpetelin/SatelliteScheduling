...
class TestEvaluator(TestCase):
    ...
    def test_evaluate15(self):
        sc = Schedule([Event(SC=1, GS=1, tStart=90, tDur=20), Event(SC=1, GS=1, tStart=150, tDur=15)])
        self.assertAlmostEqual(groundStationUsageObj(pi2, sc), 0.2430555556, places=5)

    def test_evaluate16(self):
        sc = Schedule([Event(SC=1, GS=1, tStart=90, tDur=20), Event(SC=1, GS=1, tStart=90, tDur=15)])
        self.assertAlmostEqual(groundStationUsageObj(pi2, sc), 0.1388889, places=5)
